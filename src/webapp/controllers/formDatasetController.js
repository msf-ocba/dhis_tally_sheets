...
export const formDatasetController = TallySheets.controller("formDatasetCtrl", [
    ...
    "$sce",
    ($scope, $sce) => {
        $scope.deliberatelyTrustDangerousSnippet = () => $sce.trustAsHtml($scope.outputHtml);
        ...
        $scope.formatForm = e => {
            ...
            j.find(".sectionFilter").parent().replaceWith("<th class='no-border'></th>");
            ...
            j.find(".sectionTable tbody th").parent().find("td").replaceWith("<th class='no-border'></th>");

            // Set entryfields as readonly
            j.find(".entryfield").prop("readonly", true);

            // Remove "Total" rows
            j.find(".sectionTable tr td:contains('Total')").parent("tr").remove();
            ...
            tableWithTotal.find("tbody tr").each(function () {
                if (headerSections.find("tr").find("th:contains('Total')")[0]) $(this).find("td").last().remove();
            });
            ...
            j.find(".sectionTable input:disabled").closest("td").addClass("greyfield");
            ...
            j.on("click", ".remove-section", function (event) {
                ...
                j.find("#" + sectionId).hide(400, function () {
                    ...
                    $scope.$apply(() => {
                        $scope.removedSections.push(sectionId.replace("tab-", ""));
                    });
                });
            });
        };
